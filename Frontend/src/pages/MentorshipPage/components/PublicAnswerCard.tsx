...
const PublicAnswerCard = ({ question, answer, isAuthor, onQuestionUpdate }: Props) => {
    ...
    const isOpen =
        question.status === "Unsolved" || question.status === "Pending";

    const existingThread = (question.mentorshipThreads || []).find(
        (t) =>
            answer._id &&
            t.linkedPublicAnswerId &&
            String(t.linkedPublicAnswerId) === String(answer._id)
    );

    const canOpenPrivate = isAuthor && isOpen && !existingThread;

    const handleOpenChat = async () => {
        if (!answer._id) {
            setError("This answer cannot be linked to a private chat yet. Refresh and try again.");
            return;
        }
        ...
        try {
            ...
            const msg =
                err && typeof err === "object" && "response" in err
                    ...
                    : undefined;
            setError(msg || "Could not open private chat");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="public-answer-card">
            ...
            {existingThread && isAuthor && (
                <p className="public-answer-hint thread-opened-hint">
                    <MessageSquare size={12} />
                    Private chat opened — see your private mentorship chats below.
                </p>
            )}
